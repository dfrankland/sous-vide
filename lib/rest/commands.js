...
export const SET_UNIT = unit => {
  ...
  return sendCommand(COOKER_ENDPOINT(...c()), { temp_unit: unit });
};
...
export const SET_TEMP = targetTemperature => {
  ...
  return sendCommand(COOKER_ENDPOINT(...c()), { target_temp: targetTemperature });
};

export const READ_STATUS = () => sendCommand(COOKER_ENDPOINT(...c()));

export const SET_RUNNING = running => {
  checkIsBoolean(running);
  return sendCommand(COOKER_ENDPOINT(...c()), { is_running: running });
};

export const SET_SPEAKER_MODE = mode => {
  checkIsBoolean(mode);
  return sendCommand(COOKER_ENDPOINT(...c()), { speaker_mode: mode });
};

export const READ_JOBS = () => sendCommand(JOBS_ENDPOINT(...c()));
